...
# 라벨과 가격 사이 구분자에 관대하게: 공백/줄바꿈, 대시(-, –), 콜론(:) 모두 허용
#   (\s*[-–:]?\s*). 현재 사이트는 셀 분리(=공백)지만, 표기가 'A동 - 17,700' 식으로
#   바뀌어도 안 깨지게 함. fixtures 테스트(crawler/tests)가 두 형식 모두 검증.
PATTERNS: list[tuple[str, str]] = [
    ...
    (r'폐?전선\s*\(\s*고압선?\s*A\s*\)\s*[-–:]?\s*([\d,]+)',             'wire-hv-a'),
    (r'폐?전선\s*\(\s*고압선?\s*B\s*\)\s*[-–:]?\s*([\d,]+)',             'wire-hv-b'),
    (r'폐?전선\s*\(\s*단선\s*\)\s*[-–:]?\s*([\d,]+)',                    'wire-bare'),
    (r'폐?전선\s*\(\s*통신선?\s*\)\s*[-–:]?\s*([\d,]+)',                 'wire-comm'),
    (r'폐?전선\s*\(\s*중선\s*\)\s*[-–:]?\s*([\d,]+)',                    'wire-medium'),
    (r'폐?전선\s*\(\s*잡선\s*\)\s*[-–:]?\s*([\d,]+)',                    'wire-coated'),

    # 황동(신주) — 절봉(절삭용) / 주물(주조용) 별도 매핑
    (r'황동\s*\(\s*절봉\s*\)\s*[-–:]?\s*([\d,]+)',                       'brass'),
    (r'황동\s*\(\s*주물\s*\)\s*[-–:]?\s*([\d,]+)',                       'brass-cast'),

    # 인청동 (Cu-Sn 합금) — 2종 우선
    (r'인청동\s*2종\s*[-–:]?\s*([\d,]+)',                                'phosphor-bronze'),
    (r'인청동\s*3종\s*[-–:]?\s*([\d,]+)',                                'phosphor-bronze'),
    (r'인청동\s*[-–:]?\s*([\d,]+)',                                      'phosphor-bronze'),

    # 양은 (Cu-Ni-Zn 합금) — 양백·백동 표기 변종
    (r'양은\s*[-–:]?\s*([\d,]+)',                                        'nickel-silver'),
    (r'양백\s*[-–:]?\s*([\d,]+)',                                        'nickel-silver'),
    (r'백동\s*[-–:]?\s*([\d,]+)',                                        'nickel-silver'),

    # 알루미늄 5종 — 종류별 매핑
    (r'AL\s*프로파일\s*[-–:]?\s*([\d,]+)',                               'al-profile'),
    (r'AL\s*판재\s*[-–:]?\s*([\d,]+)',                                   'al-plate'),
    (r'AL\s*샤시\s*[-–:]?\s*([\d,]+)',                                   'al-frame'),
    (r'AL\s*주물\s*[-–:]?\s*([\d,]+)',                                   'al-cast'),
    (r'AL\s*분철\s*[-–:]?\s*([\d,]+)',                                   'al-powder'),

    # 스테인리스 304 일반 (분철 자동 제외 — \s+ 다음에 숫자 직접)
    (r'STS\s*304\s*[-–:]?\s*([\d,]+)',                                   'stainless'),
    ...
    # 철 (B급 → A급 → 무등급 순)
    (r'중량\s*B\s*[-–:]?\s*([\d,]+)',                                    'iron-heavy'),
    (r'중량\s*A\s*[-–:]?\s*([\d,]+)',                                    'iron-heavy'),
    (r'경량\s*B\s*[-–:]?\s*([\d,]+)',                                    'iron-light'),
    (r'경량\s*A\s*[-–:]?\s*([\d,]+)',                                    'iron-light'),
    (r'생철\s*[-–:]?\s*([\d,]+)',                                        'iron-misc'),
    (r'잡철\s*[-–:]?\s*([\d,]+)',                                        'iron-misc'),

    # 납·배터리 (납은 폐납 등 한글 접두 제외)
    (r'(?<![가-힣])납\s*[-–:]?\s*([\d,]+)',                              'lead'),
    (r'배터리\s*[-–:]?\s*([\d,]+)',                                      'car-battery'),

    # 니켈 (99.9% 같은 등급 표기 흡수)
    (r'니켈\s*(?:99[.\d]+%?)?\s*[-–:]?\s*([\d,]+)',             'nickel'),
]
